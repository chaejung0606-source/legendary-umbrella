...
import { RotateCcw, ArrowUpRight, Eye, AlertTriangle, Loader2 } from "lucide-react";
...
// 노트북 대여현황 표. 상단에 "사용/대여 중", 아래에 "보관·기타"를 나눠 현황을 바로 파악한다.
export function LaptopLoanTable({ loans }: { loans: LaptopLoan[] }) {
  ...
  function submit(fd: FormData) {
    ...
    startTransition(async () => {
      const result =
        mode === "loan"
          ? await loanAssetAction(loan.assetId, String(fd.get("userName") ?? ""), String(fd.get("dueAt") ?? "") || null)
          : await returnAssetAction(loan.assetId, String(fd.get("afterStatus") ?? "보관") as LoanStatus, fd.get("damaged") === "on");
      if (result.ok) {
        toast({
          kind: "success",
                      ...
                      <td className="px-4 py-3.5">
                        <div className="flex justify-end gap-1.5">
                          {isOut ? (
                            ...
                            <Button size="sm" variant="outline" onClick={() => setActive({ loan: l, mode: "loan" })}><ArrowUpRight className="h-3.5 w-3.5" /> 대여</Button>
                          )}

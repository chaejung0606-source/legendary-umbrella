      ...
      <header className="sticky top-0 z-30 px-4 pt-4 sm:px-6">
        <nav className="ceramic grain mx-auto flex max-w-6xl items-center justify-between rounded-full px-4 py-2.5 sm:px-5">
          ...
          <div className="hidden items-center gap-1 md:flex">
            {NAV.map((n) => (
              <a key={n.href} href={n.href} className="rounded-full px-4 py-2 text-sm font-medium text-ceramic-sub transition hover:text-ceramic-ink">
                {n.label}
              </a>
            ))}
          </div>
          ...
          <div className="hidden md:block">
            <SoftButton variant="primary" className="px-5">시작하기</SoftButton>
          </div>
        ...
        {menuOpen && (
          <div className="mx-auto mt-2 max-w-6xl md:hidden">
            <SoftCard className="flex flex-col gap-1">
              {NAV.map((n) => (
                <a key={n.href} href={n.href} onClick={() => setMenuOpen(false)} className="rounded-2xl px-4 py-3 text-sm font-medium text-ceramic-sub transition hover:bg-ceramic-cream/60 hover:text-ceramic-ink">
                  {n.label}
                </a>
              ))}
              <SoftButton variant="primary" className="mt-1 w-full">시작하기</SoftButton>
            </SoftCard>
          </div>
        )}
      </header>
        ...
        <section id="intro" className="grid items-center gap-10 py-14 lg:grid-cols-2 lg:py-20">
          <div>
            ...
            <div className="mt-7 flex flex-col gap-3 sm:flex-row">
              <SoftButton variant="primary" className="px-7">
                시작하기 <ArrowRight className="h-4 w-4" />
              </SoftButton>
              <SoftButton variant="secondary" className="px-7">
                <PlayCircle className="h-4 w-4 text-ceramic-sage" /> 데모 보기
              </SoftButton>
            </div>
        ...
        <section id="dashboard" className="py-14 lg:py-20">
          <div className="grid items-center gap-10 lg:grid-cols-5">
            <div className="lg:col-span-2">
              ...
              <div className="mt-7">
                <SoftButton variant="primary" className="px-6">대시보드 열기 <ArrowRight className="h-4 w-4" /></SoftButton>
              </div>
            </div>
          ...
          <SoftCard large padded={false} className="relative overflow-hidden">
            ...
            <div className="relative flex flex-col items-center gap-5 px-6 py-14 text-center sm:px-10">
              ...
              <div className="flex flex-col gap-3 sm:flex-row">
                <SoftButton variant="primary" className="px-8">무료로 시작하기</SoftButton>
                <SoftButton variant="secondary" className="px-8">도입 문의</SoftButton>
              </div>
            </div>
          </SoftCard>
        </section>

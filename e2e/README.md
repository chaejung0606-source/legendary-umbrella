...
# 홍보 출고 동시성 (5 케이스) — 같은 승인 건 동시/중복 출고완료 시 재고 1회만 차감,
#   동시 승인 초과예약 방지, 단가·안전재고 음수 정규화(뮤테이션 직접 호출)
npm run e2e:promo

# 소모품 입출고 이력 (6 케이스) — 입고/출고/조정이 ConsumableTxn 이력으로 정확히 남고
#   재고 수량이 이력과 일치하는지 검증(뮤테이션 직접 호출)
